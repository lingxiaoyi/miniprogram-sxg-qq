...
import { setGlobalData, loginAndConnection, register, jumpUrl, getScene, getGlobalData } from '../../utils/wx';
...
function withLogin(lifecycle = 'willMount') {
  ...
  return function withLoginComponent(Component) {
    ...
    class WithLogin extends Component {
      ...
      async onAuthConfirmClick(type, info) {
        //注册步奏1,获取用户基本信息
        if (this.isPendding) return;
        this.isPendding = true;
        if (arguments.length === 1) {
          info = type;
        }
        let data = await register(info); //获得openid但还没有注册走这一步
        let { stat } = data;
        this.isPendding = false;
        if (stat === 0) {
          ...
          //授权登陆成功
          this.loginSuccessCallback(type); //登陆太快或者其他原因导致需要register登陆执行这个步骤
        } /* else if (stat === 2) {
          //进入手机号注册的步骤
          ...
          //jumpUrl('/pages/login/verifphone/verifphone');
        } */
         else if (stat === -5) {
          Taro.showToast({
            title: '登陆太频繁,稍等重试',
        ...
        // super.onAuthConfirmClick();//区别一下可以添加不同的日志
      }
    }
    return WithLogin;
  };
