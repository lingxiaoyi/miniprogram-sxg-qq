...
import { pddActions, promotionUrlActions } from '../../../redux/modules/details';
...
class Pdd extends Component {
  ...
  async componentDidShow() {
    ...
    // 获取商品详细信息
    await dispatch(pddActions.loadAsync(goodsId, accid));
    if (accid) {
      await this.loadPromotionUrl(accid, 'qqOwn'); // 获取转链
    }
  ...
  async loadPromotionUrl(accid, opt) {
    ...
    return this.props.dispatch(promotionUrlActions.loadPddAsync(this.goodsId, customParameters));
  }

  componentWillUnmount() {
    this.props.dispatch(pddActions.clear());
  ...
  async handleShare() {
    ...
    const { accid } = this.props.loginInfo;
    Log.click({ buttonfid: 'xq_10134', goodsid: this.goodsId, goodssource: 'pingduoduo' });
    // 生成分享链接
    /* if (!this.shareAccid) {
      // 自购进入（非分享进入）
  ...
  showSharePoster() {
    ...
    let { ownShortUrl } = this.props.promotionUrl.data || {};
  ...
  savePic() {
    Log.click({ buttonfid: 'xq_10138', goodsid: this.goodsId, goodssource: 'pingduoduo' });
  ...
  preview(e) {
    e.stopPropagation();
    Log.click({ buttonfid: 'xq_10139', goodsid: this.goodsId, goodssource: 'pingduoduo' });
    Taro.previewImage({
  ...
  onShareAppMessage() {
    ...
    // console.log(
    //   `/pages/details/pdd/pdd?id=${this.goodsId}&accid=${this.shareAccid ||
    //     this.props.loginInfo.accid ||
    //     ''}&invitecode=${myInvitecode || othersInvitecode}`
    // );
    return {
      title: goodsInfo.name,
      path: `/pages/details/pdd/pdd?id=${this.goodsId}&accid=${this.shareAccid ||
        this.props.loginInfo.accid ||
        ''}&invitecode=${myInvitecode || othersInvitecode}`,
   ...
   */
  async handleCopyTxt() {
    Log.click({ buttonfid: 'xq_10140', goodsid: this.goodsId, goodssource: 'pingduoduo' });
    if (!this.shareAccid) {
      // 自购进入（非分享进入）
      await this.loadPromotionUrl(this.props.loginInfo.accid, 'qqshare'); // 获取转链
    }
  ...
  copy() {
    ...
    Log.click({ buttonfid: 'xq_10141', goodsid: this.goodsId, goodssource: 'pingduoduo' });
  }
  formSubmit = e => {
              ...
              <View className='btns-wrap'>
                <View className='item share'>
                  ...
                  <Button
                    ...
                    className='btn'
                    onClick={() => {
                      Log.click({ buttonfid: 'x_10134', goodsid: this.goodsId, goodssource: 'pingduoduo' });
                    }}
                  />
                </View>
            ...
            >
              <View className='copy-modal'>
                ...
                <View className='content'>
                  ...
                  <Text className='item goods-oldprice'>【原价】{oldPrice}</Text>
                  <Text className='item goods-newprice'>
                    【券后价】<Text className='price'>{newPrice}</Text>
                  </Text>
...
function mapStateToProps(state) {
  const { pdd, promotionUrl } = state.details;
  //const { userInfo } = state.mine;
  const { loginInfo } = state.login;
  const { goodsGalleryUrls, goodsInfo } = pdd.data;
  return {
    //userInfo,
    loginInfo,
    goodsGalleryUrls,
...
export default connect(mapStateToProps)(Pdd);
