...
class Search extends Component {
  ...
  async componentDidMount() {
    ...
    if (this.state.showTb) {
      this.opt = 'tb';
      this.props.dispatch(searchActions.changeOpt(this.opt));
    }
        ...
        >
          ...
          <View id='searchbar' className='searchbar'>
            ...
            <View className='tabs'>
              {this.state.showTb && (
                <View
                  className={`tabs-item tabs-tb ${selectedOpt === 'tb' && 'active'}`}
                  onClick={this.handleTabsClick.bind(this, 'tb')}
                >
                  淘宝
                </View>
              )}
            ...
            {searchList && searchList.length > 0 && (
              <View className='filter-condition'>
                ...
                <View
                  className={`item commission-rate ${sortType === 1 && 'active asc'} ${sortType === 2 &&
                    'active desc'}`}
                  onClick={() => {
          ...
          <View id='search_list' className='search-list'>
            {loading && searchList && searchList.length === 0 && <Loading txt='努力搜索中...' />}
            {!loading && searchList && searchList.length === 0 && <Loading txt='~抱歉/(ㄒoㄒ)/未找到你想要的宝贝~' />}
