...
function getCountDownTime() {
  const nowTime = moment()
    .toDate()
    .getTime();
  const tm = moment().add(1, 'day');
  const tmYear = tm.year();
  const tmMonth = tm.month() + 1;
  const tmDate = tm.date();
  const entTime = moment(new Date(`${tmYear}/${tmMonth}/${tmDate} 00:00:00`))
    .toDate()
    .getTime();
  const time = entTime - nowTime;
