...
import { requestWithForm, requestWithFormMD5, request, getPublicParameters, getScene, requestWithParameters } from './api';
...
export async function loginAndConnection() {
  const loginRes = await Taro.login();
  let res = await requestWithParameters(URL.loginByOpenid, {
    code: loginRes.code,
    type: 2
  });
  setGlobalData('loginCode', loginRes.code);
  /* res = await new Promise((resolve)=>{ // 将接口延时2秒返回 测试用
      setTimeout(() => {
        resolve(res)
      }, 10000);
...
export async function register(info) {
  try {
    if (info.detail.rawData) {
      ...
      //let code = getGlobalData('loginCode');
      const loginRes = await Taro.login();
      //注册流程
      let res = await requestWithParameters(URL.otherregister, {
 ...
 * @param {*} mobile
 */
export async function getVerificationCode(mobile) {
  let res = await requestWithForm(URL.verificationCode, { mobile });
  return res.data;
}
/**
 * 通过手机号注册
...
export async function registerByPhoneVerifCode(mobile, code, invitecode = '5620061') {
  try {
    const openid = getGlobalData('sxg_openid');
    let userInfo = getGlobalData('userInfo');
    let systemInfo = getSystemInfo();
...
export async function getCloudOptions() {
  const res = await requestWithFormMD5(URL.cloudControl);
  // console.log('getCloudOptions--res>>>', res);
  return res.data;
}
